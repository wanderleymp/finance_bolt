/*
  # Implementação do Módulo de Agentes IA

  1. Novas Tabelas
    - `ai_agent_templates` - Templates pré-configurados para criação rápida de agentes
    - `ai_agent_tools` - Ferramentas que podem ser usadas pelos agentes
    - `ai_agent_conversations` - Histórico de conversas com agentes
    - `ai_agent_messages` - Mensagens individuais das conversas
    - `ai_agent_feedback` - Feedback dos usuários sobre respostas dos agentes
    - `ai_knowledge_bases` - Bases de conhecimento para os agentes
    - `ai_knowledge_documents` - Documentos nas bases de conhecimento

  2. Segurança
    - Políticas RLS para todas as tabelas
    - Permissões baseadas em tenant e papel do usuário

  3. Funções
    - Funções para gerenciamento de agentes
    - Funções para processamento de mensagens
    - Funções para gerenciamento de conhecimento
*/
