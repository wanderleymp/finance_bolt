/*
  ...
  1. Mudanças
     - Corrige a configuração do provedor Google Drive
     - Atualiza os provedores de credenciais compatíveis
     - Adiciona campos de configuração específicos para Google Drive
     - Garante que o provedor esteja ativo e disponível
  
  2. Configurações
     - Adiciona suporte para ID de pasta raiz
     - Configurações de permissões de arquivos
     - Opções de nomenclatura de arquivos
*/
...
WHERE code = 'google'
  AND NOT 'google_oauth2' = ANY(credential_providers);