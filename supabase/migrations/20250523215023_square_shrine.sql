/*
  # Fix Google Drive storage provider
  
  1. Changes
     - Removes and recreates the Google Drive storage provider with correct settings
     - Ensures proper credential provider configuration
     - Fixes array handling in SQL statements
  
  2. Provider Details
     - Configures Google Drive with appropriate settings schema
     - Sets up credential providers for authentication
*/
...
-- Atualizar o provedor Google existente para incluir o novo provedor de credenciais
-- Corrigido para usar a sintaxe correta para verificar se um valor está em um array
