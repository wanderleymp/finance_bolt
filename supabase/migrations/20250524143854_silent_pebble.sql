/*
  ...
  1. New Tables
    - `ai_agent_templates` - Templates for agent creation
    - `ai_agent_tools` - Tools that can be used by agents
    - `ai_agent_conversations` - Conversation history with agents
    - `ai_agent_messages` - Individual messages in conversations
    - `ai_agent_feedback` - User feedback on agent responses
    - `ai_knowledge_bases` - Knowledge bases for agents
    - `ai_knowledge_documents` - Documents in knowledge bases
    - `ai_document_chunks` - Document chunks for retrieval

  2. Security
    - Enable RLS on all tables
    - Add policies for proper access control
    - System-wide and tenant-specific permissions

  3. Functions
    - Process agent messages
    - Create knowledge bases for tenants
...
CREATE TABLE IF NOT EXISTS ai_document_chunks (
  ...
  metadata jsonb,
  embedding_data jsonb, -- Changed from vector type to jsonb to store embedding data
  tokens_used integer,
  chunk_index integer,
  created_at timestamptz NOT NULL DEFAULT now()
);
